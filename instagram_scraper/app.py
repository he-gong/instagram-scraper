...
#from instagram_scraper.constants import * #原文
import instagram_scraper.constants #changed for local

def doNothing(x, **argv):
	return x
tqdm.tqdm = doNothing #because tqdm.tqdm has bugs
...
class InstagramScraper(object):
	...
	def __init__(self, **kwargs):
		...
		self.session.headers = {'user-agent': instagram_scraper.constants.CHROME_WIN_UA}
	...
	def safe_get(self, *args, **kwargs):
		...
		retry = 0
		retry_delay = instagram_scraper.constants.RETRY_DELAY
		while True:
			if self.quit:
				return
			try:
				response = self.session.get(timeout=instagram_scraper.constants.CONNECT_TIMEOUT, cookies=self.cookies, *args, **kwargs)
				...
				if retry < MAX_RETRIES:
					...
					retry_delay = min(2 * retry_delay, instagram_scraper.constants.MAX_RETRY_DELAY)
					retry = retry + 1
					continue
				else:
	...
	def login(self):
		...
		self.session.headers.update({'Referer': instagram_scraper.constants.BASE_URL, 'user-agent': instagram_scraper.constants.STORIES_UA})
		req = self.session.get(instagram_scraper.constants.BASE_URL)

		self.session.headers.update({'X-CSRFToken': req.cookies['csrftoken']})

		login_data = {'username': self.login_user, 'password': self.login_pass}
		login = self.session.post(instagram_scraper.constants.LOGIN_URL, data=login_data, allow_redirects=True)
		...
		if login_text.get('authenticated') and login.status_code == 200:
			...
			if 'checkpoint_url' in login_text:
				...
				self.logger.error('Please verify your account at ' + instagram_scraper.constants.BASE_URL[0:-1] + checkpoint_url)
	...
	def login_challenge(self, checkpoint_url):
		self.session.headers.update({'Referer': instagram_scraper.constants.BASE_URL})
		req = self.session.get(instagram_scraper.constants.BASE_URL[:-1] + checkpoint_url)
		self.session.headers.update({'X-CSRFToken': req.cookies['csrftoken'], 'X-Instagram-AJAX': '1'})

		self.session.headers.update({'Referer': instagram_scraper.constants.BASE_URL[:-1] + checkpoint_url})
		mode = int(input('Choose a challenge mode (0 - SMS, 1 - Email): '))
		challenge_data = {'choice': mode}
		challenge = self.session.post(instagram_scraper.constants.BASE_URL[:-1] + checkpoint_url, data=challenge_data, allow_redirects=True)
		...
		code = self.session.post(instagram_scraper.constants.BASE_URL[:-1] + checkpoint_url, data=code_data, allow_redirects=True)
	...
	def logout(self):
		...
		if self.logged_in:
			try:
				...
				self.session.post(instagram_scraper.constants.LOGOUT_URL, data=logout_data)
				self.logged_in = False
	...
	def get_last_scraped_timestamp(self, username):
		if self.latest_stamps_parser:
			try:
				return self.latest_stamps_parser.getint(instagram_scraper.constants.LATEST_STAMPS_USER_SECTION, username)
	...
	def set_last_scraped_timestamp(self, username, timestamp):
		if self.latest_stamps_parser:
			if not self.latest_stamps_parser.has_section(instagram_scraper.constants.LATEST_STAMPS_USER_SECTION):
				self.latest_stamps_parser.add_section(instagram_scraper.constants.LATEST_STAMPS_USER_SECTION)
			self.latest_stamps_parser.set(instagram_scraper.constants.LATEST_STAMPS_USER_SECTION, username, str(timestamp))
	...
	def __query_comments(self, shortcode, end_cursor=''):
		params = instagram_scraper.constants.QUERY_COMMENTS_VARS.format(shortcode, end_cursor)
		self.update_ig_gis_header(params)

		resp = self.get_json(instagram_scraper.constants.QUERY_COMMENTS.format(params))
	...
	def __scrape_query(self, media_generator, executor=concurrent.futures.ThreadPoolExecutor(max_workers=instagram_scraper.constants.MAX_CONCURRENT_DOWNLOADS)):
			...
			for value in self.usernames:
				...
				for item in tqdm.tqdm(media_generator(value), desc='Searching {0} for posts'.format(value), unit=" media",
					...
					if ((item['is_video'] is False and 'image' in self.media_types) or \
						...
						) and self.is_new_media(item, dst):
				...
				if (self.media_metadata or self.comments or self.include_location) and self.posts:
					...
					for item in self.posts:
						self.save_json(item, '{0}/{1}.json'.format(dst, item["id"]))
		finally:
			self.quit = True

	def query_hashtag_gen(self, hashtag):
		return self.__query_gen(instagram_scraper.constants.QUERY_HASHTAG, instagram_scraper.constants.QUERY_HASHTAG_VARS, 'hashtag', hashtag)

	def query_location_gen(self, location):
		return self.__query_gen(instagram_scraper.constants.QUERY_LOCATION, instagram_scraper.constants.QUERY_LOCATION_VARS, 'location', location)
	...
	def __get_media_details(self, shortcode):
		resp = self.get_json(instagram_scraper.constants.VIEW_MEDIA_URL.format(shortcode))
	...
	def scrape(self, executor=concurrent.futures.ThreadPoolExecutor(max_workers=instagram_scraper.constants.MAX_CONCURRENT_DOWNLOADS)):
		"""Crawls through and downloads user's media"""
		self.session.headers = {'user-agent': instagram_scraper.constants.STORIES_UA}
		try:
	...
	def get_profile_pic(self, dst, executor, future_to_item, user, username):
		...
		url = instagram_scraper.constants.USER_INFO.format(user['id'])
	...
	def get_shared_data(self, username=''):
		...
		resp = self.get_json(instagram_scraper.constants.BASE_URL + username)
	...
	def fetch_stories(self, user_id):
		...
		resp = self.get_json(instagram_scraper.constants.STORIES_URL.format(user_id))
	...
	def __query_media(self, id, end_cursor=''):
		params = instagram_scraper.constants.QUERY_MEDIA_VARS.format(id, end_cursor)
		self.update_ig_gis_header(params)

		resp = self.get_json(instagram_scraper.constants.QUERY_MEDIA.format(params))
	...
	def download(self, item, dst='./'):
		itemname = '{0}'.format(item["id"])
		#itemname = '{1}_{2}'.format(item["id"], item["shortcode"])
		self.save_json(item, '{0}/{1}.json'.format(dst, itemname))
		...
		for full_url, base_name in self.templatefilename(item):
			...
			_, fextension = os.path.splitext(base_name)
			if(num == 0):
				file_path = os.path.join(dst, '{item}.{ext}'.format(item=itemname, ext=fextension))
			else:
				file_path = os.path.join(dst, '{item}_{order}.{ext}'.format(item=itemname, order=num, ext=fextension))
					...
					try:
						...
						while(True):
							...
							try:
								...
								with self.session.get(url, cookies=self.cookies, headers=headers, stream=True, timeout=instagram_scraper.constants.CONNECT_TIMEOUT) as response:
									if response.status_code == 404:
	...
	def is_new_media(self, item, dst = './'):
		"""Returns True if the media is new."""
		if(os.path.exists('{0}/{1}_{2}.json'.format(dst, item["id"], item["shortcode"]))):
			return False
	...
	@staticmethod
	def __search(query):
		resp = requests.get(instagram_scraper.constants.SEARCH_URL.format(query))
...
def main():
	...
	parser.add_argument('--item-template', type=str, default='{username}_{shortcode}', help='Customize filename template')
